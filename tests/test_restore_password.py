...
class TestRestorePassword:
    ...
    @allure.description('Открываем страницу логина https://stellarburgers.nomoreparties.site/login, '
                        'кликаем на кнопку «Восстановить пароль», ожидаем переход на страницу восстановления '
                        'пароля https://stellarburgers.nomoreparties.site/forgot-password')
    def test_transfer_from_restore_button_to_restore_page(self, driver):
        ...
        assert restore_page.get_current_url(Urls.PASSWORD_RESTORE_PAGE)

    @allure.title('Проверяем, что ввод почты и клик по кнопке «Восстановить» переводит на страницу восстановления пароля')
    @allure.description('На странице https://stellarburgers.nomoreparties.site/forgot-password вводим почту в окно '
                        'Email и нажимаем кнопку «Восстановить», ожидаем переход на страницу '
    ...
    def test_input_email_click_restore_button(self, driver):
        ...
        restore_page.set_email(D.EMAIL)
        restore_page.click_restore_button()
        assert restore_page.get_current_url(Urls.RESET_PASSWORD_PAGE)

    @allure.title('Проверяем, что клик по кнопке показать/скрыть пароль делает поле активным — подсвечивает его.')
    @allure.description('На странице https://stellarburgers.nomoreparties.site/forgot-password вводим почту в окно '
                        'Email и нажимаем кнопку «Восстановить», переходим на страницу '
                        'https://stellarburgers.nomoreparties.site/reset-password '
                        'нажимаем на кнопку показать пароль - ожидаем, что поле пароль становится активное')
    def test_show_hide_password_on_restore_page(self, driver):
        ...
        assert restore_page.find_element(RestoreLoc.RESTORE_MAIL_FIELD_ACTIVE)
