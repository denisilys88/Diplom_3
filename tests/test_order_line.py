...
class TestOrderLine:
    ...
    @allure.description('Открываем главную страницу https://stellarburgers.nomoreparties.site'
                        'нажимаем кнопку ленты заказов, в ленте заказов нажимаем на заказ '
    ...
    def test_show_order_details(self, driver):
        ...
        assert main_page.find_order_details()
    ...
    @allure.description('Открываем страницу https://stellarburgers.nomoreparties.site/login, логинимся'
                        'оформляем заказа, получаем номер заказа, закрываем окно с заказом и нажимаем на кнопку '
                        '"Лента заказов", проверяем, что номер нашего заказа есть в ленте заказов')
    def test_show_orders_on_order_line(self, login):
        page = PersonalAreaPage(login)
        page.drag_ingredient_to_order(MainLoc.ORDER_LINE_ELEMENT_BUN)
        page.drag_ingredient_to_order(MainLoc.ORDER_LINE_ELEMENT_SAUCE)
        ...
        assert page.get_made_order_number_in_order_line()[2:] == made_order_number

    @allure.title('Проверяем, что при создании нового заказа счётчик Выполнено за всё время увеличивается')
    @allure.description('Открываем страницу https://stellarburgers.nomoreparties.site/login, логинимся'
                        'со страницы ленты заказов фиксируем общий счетчик заказов "Выполнено за все время"'
                        'на главной странице оформляем заказ, нажимаем на ленту заказов, '
                        'проверяем, что счетчик Выполнено за всё время увеличился на 1')
    def test_add_order_alltime_counter_increase(self, login):
        ...
        page.click_close_details_button()
        page.click_order_line_button()
        assert int(page.get_alltime_order_count()) == int(alltime_counter) + 1

    @allure.title('Проверяем, что при создании нового заказа счётчик Выполнено за сегодня увеличивается')
    @allure.description('Открываем страницу https://stellarburgers.nomoreparties.site/login, логинимся'
                        'со страницы ленты заказов фиксируем счетчик заказов за сегодня "Выполнено за сегодня"'
                        'на главной странице оформляем заказ, нажимаем на ленту заказов, '
                        'проверяем, что счетчик "Выполнено за сегодня" увеличился на 1')
    def test_add_order_day_counter_increase(self, login):
        ...
        page.click_close_details_button()
        page.click_order_line_button()
        assert int(page.get_daily_order_count()) == int(daily_counter) + 1

    @allure.title('Проверяем, что после оформления заказа его номер появляется в разделе В работе')
    @allure.description('Открываем страницу https://stellarburgers.nomoreparties.site/login  логинимся '
                        'оформляем заказ, переходим в ленту заказов,проверяем, что его номер заказа '
    ...
    def test_add_order_show_in_work(self, login):
        ...
        page.click_close_details_button()
        page.click_order_line_button()
        assert page.get_order_in_work_number()[1:] == made_order_number
