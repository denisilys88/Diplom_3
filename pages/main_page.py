...
class MainPage(BasePage):
    ...
    def click_make_order_button(self):
        return self.find_element(MainLoc.MAKE_ORDER_BUTTON).click()

    @allure.step('Находим элемент оформленного заказа')
    def find_made_order_pop_up(self):
        return self.find_element(MainLoc.MADE_ORDER_POP_UP)

    @allure.step('Находим номер оформленного заказа')
    def get_made_order_pop_up_number(self):
        return self.find_element(MainLoc.MADE_ORDER_POP_UP_NUMBER).text

    @allure.step('Находим номер оформленного заказа')
    def get_made_order_number_in_order_line(self):
        return self.find_element(MainLoc.MADE_ORDER_IN_ORDER_LINE).text

    @allure.step('Находим номер оформленного заказа')
    def get_alltime_order_count(self):
        return self.find_element(MainLoc.ALLTIME_ORDER_COUNTER).text

    @allure.step('Находим номер оформленного заказа')
    def get_daily_order_count(self):
        return self.find_element(MainLoc.DAILY_ORDER_COUNTER).text

    @allure.step('Находим номер заказа в работе')
    def get_order_in_work_number(self):
        return self.find_element(MainLoc.ORDER_IN_WORK).text
